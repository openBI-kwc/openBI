...
class Chart
{
    ...
    public function index(string $type  , array $data)
    {
        ...
        //声明图表类型变量
        $charttype = $charttype = config('chartType.'.$type);
        if($charttype != '') {
            //将数据传入处理方法内
    ...
    //bar1
    protected function huxingzhutu(array $data) 
    {
        //定义新存储数组
        $datas = [];
        $i = 0;
    ...
    //scatter3
    protected function scatter3_black(array $data)
    {
        //定义存储数组datas
        $datas = [];
        $line = [];
        ...
        foreach ($data as $key => $value) {
            //判断是否有'x','y','z'字段
           if($this->required($value,['x','y','z','series'])) {
                continue;
           }
        ...
        //设置格式
        $datas['series'][] = ['seriesType' => 'scatter','seriesName' => $value['series'] , 'scatter' => $scatter ];
        $datas['series'][] = ['seriesType' => 'line','seriesName' => $value['series'] , 'scatter' => $line ];
    ...
    protected function scatter3(array $data)
    {
        ...
        foreach ($data as $key => $value) {
           ...
           $line[$value['series']] = ['seriesType' => 'scatter','seriesName' => $value['series']];
           $tmp[$value['series']][] = ['value' => [$value['x'] , $value['y']]];
           $line[$value['series']]['scatter'] = $tmp[$value['series']];
           $scatter[$value['series']] = ['seriesType' => 'line','seriesName' => $value['series']];
           $tmp2[$value['series']][] = ['value' => [$value['x'], $value['z']]];
           $scatter[$value['series']]['scatter'] = $tmp2[$value['series']];
        }
        //判断是否有符合的值
        if (empty($line) || empty($scatter) || empty($tmp) || empty($tmp2))  {
            return $datas;
        }
        ...
        //设置格式
        $datas['series'] = array_merge($scatter , $line);
    ...
    protected function scatter4(array $data)
    {
        ...
        foreach ($data as $key => $value) {
            //判断是否有'x','y','z'字段
            if($this->required($value,['x','y','z','r','series'])) {
                continue;
            }
    ...
    //百分比占比图
    protected function baifenbiduibibintu(array $data)
    {
        //定义返回数组
        $datas = [];
        //设置遍历次数
    ...
    //进度条形码
    protected function jindutiaoxingma( array  $data)
    {
        //定义返回数组
        $datas = [];
        //遍历数组
    ...
    //进度条形码
    public function jianbianzhuzhuangtu( array  $data)
    {
        //定义返回数组
        $datas = [];
        //遍历数组
    ...
    //3d柱状趋势变化图
    protected function zhuzhuangqushibianhuatu3d(array  $data)
    {
        //定义返回数组
        $datas = [];
    ...
    //水平发光柱状图
    public function shuipingfaguangzhuzhuangtu( array  $data)
    {
        //定义返回数组
        $datas = [];
        //遍历数组
    ...
    //图表类型linebar
    protected function timelinebar(array $data) 
    {
        //声明数组,存储分组后的数据
        $datas = [];
        //存放季度
        ...
        foreach($data as $key => $value) {
            ...
            // dump($value);
            //判断value中是否有name 和value 键
            if ($this->required($value,['value','name','lengend'])) {
                //不满足条件则不返回
                continue;
            } 
    ...
    //折线柱状图
    public function zhexianzhuzhuangtu(array $data)
    {
    ...
    //雷达bia变形图
    public function leidabianxingtu(array $data)
    {
        $datas = [];
        $target = [];
        $maxTargetValue = [];
    ...
    //数据饼图
    public function shujubingtu(array $data)
    {
        //定义返回数组
        $datas = [];
        //遍历数组
    ...
    //环形数据图
    public function huanxingshujutu(array $data)
    {
        $datas = [];
        //遍历数组
        foreach ($data as $key => $value) {
    ...
    //环形比例图
    public  function huanxingbilitu(array $data)
    {
        //定义返回数组
        $datas = [];
        //遍历数组
    ...
    //八边形图2
    public function babianxingtutwo(array $data)
    {
        //定义返回数组
        $datas = [];
        //遍历数组
    ...
    //四叶草图
    public function siyecaotu(array $data)
    {
        //定义返回数组
        $datas = [];
        //遍历数组
    ...
    //原样返回类型
    public function asItIs(array $data)
    {
        return $data;
    }

    //gis地图第一版
    public function kxiantu2(array $data)
    {
        //定义返回数组
        $datas = [];
        foreach ($data as $key => $value) {
            //判断数据是否有键'time','volumes','value'
            if($this->required($value,['time','volumes','value'])) {
                continue;
            }
            $datas[$key][] = $value['time'];
            foreach ($value['value'] as $v) {
                $datas[$key][] = $v;
            }
            $datas[$key][] = $value['volumes'];
        }
        //返回数据转换成索引数组
    ...
    {
        //接收base64图片编码
         $data = Db::name('chartdata')->where('charttype',"kxiantu2")->find();

        $data = json_decode($data['data'],1);
        //  dump($data);die;
         //定义返回数组
        $datas = $this->index('kxiantu2',$data);
        return $datas;
        die;
